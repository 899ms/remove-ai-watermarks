...
**GPT Image 2.5 compatibility, verified 2026-09-15.** One fresh low-quality
1024 x 1024 API generation from each released model, `gpt-image-2.5-flare` and
`gpt-image-2.5-sunburst`, carried a valid bound OpenAI C2PA claim generated by
`OpenAI Media Service API`, with `trainedAlgorithmicMedia` and the action set
`created, converted, watermarked.unbound`. `identify` reported high-confidence
